...
const Problems = () => {
  ...
  const handleAddTable = () => {
    if (newTableName) {
      try {
        ProblemsDB.createTable({ tableName: newTableName, primaryKey: "id" });
        handleAddPopupClose();
      } catch (error: any) {
  ...
  const handleAddEntry = () => {
    ProblemsDB.insert(ItemAdder.name, { ...ItemAdder.value, status: false });
    handleAddClose();
  };
  return (
    <div className="container">
      {ProblemsDB &&
        ProblemsDB.listTables().map((tableName, index) => {
          const tableData = ProblemsDB.select(tableName);
          return (
            <div
              className="item"
              key={index}
